...
class SenderImpatientReceiverRnnReinforce(nn.Module):
    ...
    def forward(self, sender_input, labels, receiver_input=None):
        ...
        # Loss by output
        loss, rest, crible_acc = self.loss(sender_input, message, message_lengths, receiver_input, receiver_output[:,rand_length,:], labels)
...
class CompositionalitySenderImpatientReceiverRnnReinforce(nn.Module):
    ...
    def forward(self, sender_input, labels, receiver_input=None):
        ...
        self.length_cost = 0.2  # Constant length cost
        length_loss = message_lengths.float() * self.length_cost
